...
def create_obj(frame,master,label):
	label.destroy()
	gfg = GFG(frame)
...
if __name__ == "__main__":
	master = Tk(className="Python")
	master.geometry("600x700")
	master.resizable(False,False)

	frame = Frame(master,height=500,bg="black",width=600)
	label = Label(frame,text="Snake Game by Sushant lama",bg="black",foreground="white")
	label.place(relx=0.5,rely=0.5,anchor=CENTER)
	frame.pack()
	btn = Button(master, text='Start', width=40,height=2, bd='10', command=lambda:create_obj(frame,master,label))
	btn.pack()
